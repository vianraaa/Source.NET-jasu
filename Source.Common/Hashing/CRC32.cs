...
public static class CRC32
{
	...
	public const CRC32_t CRC32_XOR_VALUE = 0xFFFFFFFFU;
	public static readonly CRC32_t[] pulCRCTable = [
	...
	public static void Init(ref CRC32_t pulCRC) => pulCRC = CRC32_INIT_VALUE;
	...
	JustAfew:
		switch (nBuffer) {
			case 7:
				ulCrc = pulCRCTable[*pb++ ^ (byte)ulCrc] ^ ulCrc >> 8;
				goto case 6;
			case 6:
				ulCrc = pulCRCTable[*pb++ ^ (byte)ulCrc] ^ ulCrc >> 8;
				goto case 5;
			...
			case 4:
				ulCrc ^= *(CRC32_t*)pb;
				ulCrc = pulCRCTable[(byte)ulCrc] ^ ulCrc >> 8;
				ulCrc = pulCRCTable[(byte)ulCrc] ^ ulCrc >> 8;
				ulCrc = pulCRCTable[(byte)ulCrc] ^ ulCrc >> 8;
				ulCrc = pulCRCTable[(byte)ulCrc] ^ ulCrc >> 8;
				pulCRC = ulCrc;
	...
	public static unsafe CRC32_t ProcessSingleBuffer(void* data, nint len) {
		CRC32_t ret = default;

		Init(ref ret);
		ProcessBuffer(ref ret, data, (int)len);
		Final(ref ret);
