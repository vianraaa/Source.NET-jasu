...
namespace Source.Common.Formats.Keyvalues
{
	...
	{
		public static TObject MakeObject<TObject>(KeyValues keyValueObject)
			=> MakeObject<TObject>(keyValueObject, new DefaultObjectReflector());

		public static object MakeObject(Type objectType, KeyValues keyValueObject, IObjectReflector reflector)
			=> InvokeGeneric(nameof(MakeObject), objectType, new object[] { keyValueObject, reflector });

		public static TObject MakeObject<TObject>(KeyValues keyValueObject, IObjectReflector reflector) {
		...
		public static KeyValues FromObject(Type objectType, object managedObject, string topLevelName)
			=> FromObjectCore(objectType, managedObject, topLevelName, new DefaultObjectReflector(), new HashSet<object>());

		static KeyValues FromObjectCore(Type objectType, object managedObject, string topLevelName, IObjectReflector reflector, HashSet<object> visitedObjects) {
			...
			return new KeyValues(topLevelName, transformedValue);
		}
		...
		static KVValue ConvertObjectToValue(Type objectType, object managedObject, IObjectReflector reflector, HashSet<object> visitedObjects) {
			...
			if (typeof(IDictionary).IsAssignableFrom(objectType)) {
				...
				while (enumerator.MoveNext()) {
					...
					childObjects.Add(new KeyValues(entry.Key.ToString(), childObjectValue));
				}
			}
		...
		static KeyValues CopyObject(object @object, string name, IObjectReflector reflector, HashSet<object> visitedObjects)
			=> FromObjectCore(@object.GetType(), @object, name, reflector, visitedObjects);

		static void CopyObject(KeyValues kv, object obj, IObjectReflector reflector) {
		...
		static bool IsArray(KeyValues obj, out KVValue[] values) {
			values = null;
		...
		static object MakeLookup(Type valueType, IEnumerable<KeyValues> items, IObjectReflector reflector)
			=> InvokeGeneric(nameof(MakeLookupCore), valueType, new object[] { items, reflector });

		static ILookup<string, TValue> MakeLookupCore<TValue>(IEnumerable<KeyValues> items, IObjectReflector reflector)
		...
		static object MakeDictionary(Type type, KeyValues kv, IObjectReflector reflector) {
		...
		static void FillDictionary<TKey, TValue>(Dictionary<TKey, TValue> dictionary, KeyValues kv, IObjectReflector reflector) {
			foreach (var item in kv.Children) {
		...
		static object ConvertValue(object value, Type valueType, IObjectReflector reflector) {
			if (value is KVCollectionValue collectionValue) {
				return MakeObject(valueType, new KeyValues("boo", (KVValue)collectionValue), reflector);
			}
		...
		static bool TryConvertValueTo<TValue>(string name, object value, out TValue converted) {
			if (typeof(TValue) == typeof(nint)) {
				converted = (TValue)(object)(nint)(KVValue)value;
				return true;
			}
		...
		static KVValue ConvertToKVValue(object value, Type type) {
			...
			return Type.GetTypeCode(type) switch {
				...
				TypeCode.Int16 => (KVValue)(short)value, // There is no int16 kv type
