...
public class NET_Tick : NetMessage
{
	...
	public override bool ReadFromBuffer(bf_read buffer) {
		...
		HostFrameTime = (float)buffer.ReadUBitLong(16) / NET_TICK_SCALEUP;
		HostFrameDeviation = (float)buffer.ReadUBitLong(16) / NET_TICK_SCALEUP;
...
public class svc_UpdateStringTable : NetMessage
{
	...
	public override bool ReadFromBuffer(bf_read buffer) {
		TableID = (int)buffer.ReadUBitLong(Protocol.MAX_TABLES_BITS);
